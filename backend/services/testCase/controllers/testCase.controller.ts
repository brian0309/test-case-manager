...
export const getTestCasesBySuite = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (isPaginatedRequest) {
      ...
      const result = await testCaseService.getTestCasesBySuitePaginated(suiteId, userId, {
        limit: safeLimit,
        offset: safeOffset,
      });
      const responses = result.items.map(testCaseService.formatTestCaseResponse);
...
export const getTestCasesByProject = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (isPaginatedRequest) {
      const safeLimit = Math.min(limit as number, 200);
      const safeOffset = offset as number;
      const result = await testCaseService.getTestCasesByProjectPaginated(projectId, userId, {
        limit: safeLimit,
        offset: safeOffset,
      });
      const responses = result.items.map(testCaseService.formatTestCaseResponse);
