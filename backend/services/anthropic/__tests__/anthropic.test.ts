...
import anthropicRoutes from '../routes/anthropic.route.js';
...
const setupUserMock = (userId: string, anthropicApiKey: string | undefined, extra: Record<string, unknown> = {}) => {
    const userDoc: any = {
...
describe('Anthropic Integration', () => {
    ...
    describe('Encryption Service', () => {
        it('should encrypt and decrypt an API key correctly', () => {
            const originalKey = 'sk-ant-test-123';
    ...
    describe('API Endpoints', () => {
        it('should save the API key encrypted', async () => {
            ...
            expect(updateArg.anthropicApiKey).toBeDefined();
            expect(updateArg.anthropicApiKey).not.toBe(apiKey);
            expect(decryptApiKey(updateArg.anthropicApiKey)).toBe(apiKey);
        });
        ...
        it('should return settings without leaking the API key', async () => {
            ...
            const res = await request(app)
                .get('/api/anthropic/settings?refresh=1')
                .set('Cookie', cookie);
