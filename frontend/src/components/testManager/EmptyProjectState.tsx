...
const EmptyProjectState: React.FC<EmptyProjectStateProps> = ({ title, description }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full text-center px-4">
            ...
            <p className="text-sm text-gray-500 mb-6 max-w
-md">{description}</p>
            <button
                onClick={() => navigate('/test-manager/projects')}
                className="px-4 py-2 bg-[#007AFF] hover:bg-[#0062cc] text-white rounded-lg transition-colors text-sm font-medium shadow-sm"
            >
                Go to Projects
            </button>
        </div>
